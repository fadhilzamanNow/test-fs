...
<template>
    <RouterLink :to="mode" :class="['relative flex items-center py-2 px-3 my-1 font-medium cursor-pointer  rounded-md' , !expand && 'justify-center gap-0 group', path.params.module === mode ? 'bg-black text-white' : 'hover:bg-slate-200']"   >
        ...
        <span v-if="!expand" class="hidden  absolute z-[9999999999999999999999999999] left-25 top-1/2 -translate-y-1/2 bg-black text-white  px-4 size-10 rounded-md sm:flex justify-start items-center w-max invisible opacity-0 transition-all group-hover:visible group-hover:opacity-100">{{ label }}</span>
    </RouterLink>
</template>
